...
var ASM_CONSTS = {
  1453172: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1453338: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1453438: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 1453632: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 1453728: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 1454259: ($0) => { var adtype = UTF8ToString($0); const callbacks = ({ adFinished: () => ccall('crazygamesad_finished_callback', null), adError: (error) => ccall('crazygamesad_error_callback', null, ['string'], [error ? error.message : ""]), adStarted: () => ccall('crazygamesad_started_callback', null) }); window.CrazyGames.SDK.ad.requestAd(adtype, callbacks); },  
 1454607: () => { window.CrazyGames.SDK.game.happytime(); },  
 1454647: () => { window.CrazyGames.SDK.game.gameplayStart(); },  
 1454691: () => { window.CrazyGames.SDK.game.gameplayStop(); },  
 1454734: () => { window.CrazyGames.SDK.game.sdkGameLoadingStart(); },  
 1454784: () => { window.CrazyGames.SDK.game.sdkGameLoadingStop(); },  
