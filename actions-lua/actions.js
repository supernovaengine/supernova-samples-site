...
// end include: /tmp/tmp5d3a_l0f.js
...
var ASM_CONSTS = {
  1922724: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1922890: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1922939: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1922988: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1923088: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 1923282: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 1923378: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 1930367: ($0) => { miniaudio.untrack_device_by_index($0); },  
 1930410: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 1930535: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
