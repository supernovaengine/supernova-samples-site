...
// end include: /tmp/tmpew4gwthh.js
...
var ASM_CONSTS = {
  1922972: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1923138: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1923187: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1923236: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1923336: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 1923530: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 1923626: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 1924157: ($0) => { var adtype = UTF8ToString($0); const callbacks = ({ adFinished: () => ccall('crazygamesad_finished_callback', null), adError: (error) => ccall('crazygamesad_error_callback', null, ['string'], [error ? error.message : ""]), adStarted: () => ccall('crazygamesad_started_callback', null) }); window.CrazyGames.SDK.ad.requestAd(adtype, callbacks); },  
 1924505: () => { window.CrazyGames.SDK.game.happytime(); },  
 1924545: () => { window.CrazyGames.SDK.game.gameplayStart(); },  
 1924589: () => { window.CrazyGames.SDK.game.gameplayStop(); },  
 1924632: () => { window.CrazyGames.SDK.game.sdkGameLoadingStart(); },  
 1924682: () => { window.CrazyGames.SDK.game.sdkGameLoadingStop(); },  
