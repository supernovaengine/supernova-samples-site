...
// end include: /tmp/tmpn_jnccql.js
...
var ASM_CONSTS = {
  1999688: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1999854: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1999903: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1999952: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2000052: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 2000246: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 2000342: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
