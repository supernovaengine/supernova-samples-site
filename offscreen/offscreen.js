...
// end include: /tmp/tmp3o37y5_x.js
...
var ASM_CONSTS = {
  1923060: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1923226: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1923275: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1923324: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1923424: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 1923618: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 1923714: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 1923776: () => { function loadJS(FILE_URL, async = true) { let scriptEle = document.createElement("script"); scriptEle.setAttribute("src", FILE_URL); scriptEle.setAttribute("type", "text/javascript"); scriptEle.setAttribute("async", async); document.body.appendChild(scriptEle); scriptEle.addEventListener("load", () => { }); scriptEle.addEventListener("error", (ev) => { console.log("Error on loading file", ev); }); } loadJS("https://sdk.crazygames.com/crazygames-sdk-v2.js", false); },  
 1924245: ($0) => { var adtype = UTF8ToString($0); const callbacks = ({ adFinished: () => ccall('crazygamesad_finished_callback', null), adError: (error) => ccall('crazygamesad_error_callback', null, ['string'], [error ? error.message : ""]), adStarted: () => ccall('crazygamesad_started_callback', null) }); window.CrazyGames.SDK.ad.requestAd(adtype, callbacks); },  
 1924593: () => { window.CrazyGames.SDK.game.happytime(); },  
 1924633: () => { window.CrazyGames.SDK.game.gameplayStart(); },  
 1924677: () => { window.CrazyGames.SDK.game.gameplayStop(); },  
 1924720: () => { window.CrazyGames.SDK.game.sdkGameLoadingStart(); },  
 1924770: () => { window.CrazyGames.SDK.game.sdkGameLoadingStop(); },  
