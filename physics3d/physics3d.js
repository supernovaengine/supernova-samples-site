...
// end include: /tmp/tmp_xq20_ol.js
...
var ASM_CONSTS = {
  1803724: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1803890: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1803939: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1803988: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1804088: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 1804282: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 1804378: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 1811367: ($0) => { miniaudio.untrack_device_by_index($0); },  
 1811410: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 1811535: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
