...
// end include: /tmp/tmppqq56o6h.js
...
var ASM_CONSTS = {
  2002728: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2002894: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 2002943: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 2002992: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2003092: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 2003286: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 2003382: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 2010371: ($0) => { miniaudio.untrack_device_by_index($0); },  
 2010414: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 2010539: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
