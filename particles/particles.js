...
// end include: /tmp/tmpdhry6gv_.js
...
var ASM_CONSTS = {
  2002472: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2002638: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 2002687: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 2002736: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2002836: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 2003030: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 2003126: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
