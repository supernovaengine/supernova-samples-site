...
// end include: /tmp/tmpkeuny2u7.js
...
var ASM_CONSTS = {
  1999768: () => { FS.mkdir('/datafs'); FS.mount(IDBFS, {}, '/datafs'); FS.syncfs(true, function(err) { ccall('syncfs_enable_callback', null, ['string'], [err ? err.message : ""]); }); },  
 1999934: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 1999983: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 2000032: () => { FS.syncfs(function(err) { ccall('syncfs_callback', null, ['string'], [err ? err.message : ""]); }); },  
 2000132: ($0) => { var key = UTF8ToString($0); var val = localStorage.getItem(key); if (val) { var len = lengthBytesUTF8(val)+1; var buf = _malloc(len); stringToUTF8(val, buf, len); return buf; } return null; },  
 2000326: ($0, $1) => { var key = UTF8ToString($0); var value = UTF8ToString($1); localStorage.setItem(key, value); },  
 2000422: ($0) => { var key = UTF8ToString($0); localStorage.removeItem(key); },  
 ...
 2007411: ($0) => { miniaudio.untrack_device_by_index($0); },  
 2007454: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 2007579: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
